    ...
    if (formElements) {
        ...
        for (let i = 0; i < formElements.length; i++) {
            if(formElements[i].hasAttribute('name')){
                if(formElements[i].hasAttribute('type') && formElements[i].getAttribute('type') === 'radio' && formElements[i].checked){
                    dataName = formElements[i].getAttribute('name');
                    dataValue = formElements[i].value;
                    data[dataName] = dataValue;
                } else if(formElements[i].hasAttribute('type') && formElements[i].getAttribute('type') !== 'radio'){
                    dataName = formElements[i].getAttribute('name');
                    dataValue = formElements[i].value;
                    data[dataName] = dataValue;
                }
